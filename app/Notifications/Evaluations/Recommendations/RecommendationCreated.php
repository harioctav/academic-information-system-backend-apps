...
class RecommendationCreated extends Notification
{
  ...
  public function toArray(object $notifiable): array
  {
    ...
    $message = $subjectCount === 1
      ? "{$this->creator->name} ({$creatorRole}) telah menambahkan rekomendasi matakuliah {$subjectList} untuk mahasiswa {$this->student->name} ({$this->student->nim})"
      : "{$this->creator->name} ({$creatorRole}) telah menambahkan {$subjectCount} rekomendasi matakuliah ({$subjectList}) untuk mahasiswa {$this->student->name} ({$this->student->nim})";

    return [
      'recommendations' => $this->recommendations->pluck('id'),
